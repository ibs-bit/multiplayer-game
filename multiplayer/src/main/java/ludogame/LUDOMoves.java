...
public class LUDOMoves extends JPanel implements KeyListener, ActionListener,MouseListener{
	...
	public LUDOMoves() {
        ...
        requestFocus();
        current_player=0;
        la = new Interface(80,50);
        p=new Games(la.height,la.width);
        dice=0;
        flag=0;
        roll=0;
        kill=0;
    ...
    @Override
    public void paint(Graphics grafity) {
    	la.draw((Graphics2D)grafity);
    	p.draw((Graphics2D)grafity);
    	if(p.pl[current_player].coin==4) {
    		grafity.setColor(Color.WHITE);
    		grafity.fillRect(590, 100, 380,130);
    		if(current_player==0) {
            ...
            grafity.drawString("Player "+(current_player+1)+" wins.", 600, 150);
            grafity.drawString("Congratulations.", 600, 200);
            current_player=0;
            la = new Interface(80,50);
            p=new Games(la.height,la.width);
            dice=0;
            ...
            grafity.setFont(new Font("serif", Font.BOLD, 40));
            grafity.drawString("Player "+(current_player+1), 600, 150);
            grafity.drawString("Number on dice is "+dice, 600, 200);
    	}
			...
			if(dice==6) {
				...
				if(value!=-1) {
					...
					for(int i=0;i<4;i++) {
						if(i!=current_player) {
							for(int j=0;j<4;j++) {
								int tem1=ShortestWay.ax[current_player][p.pl[current_player].pa[value].current],tem2=ShortestWay.ay[current_player][p.pl[current_player].pa[value].current];
