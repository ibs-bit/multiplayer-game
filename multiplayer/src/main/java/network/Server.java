...
public class Server {
  /*  private int port;
    static Pawn[][] board = new Pawn[15][15];
    ...
    public static void main(String[] args) throws Exception {
            for (int j = 0; j < 8; j++) {
                for (int i = 0; i < 2; i++) {
                    board[i][j] = new Pawn(i, j, blue, board);
                }
                for (int i = 6; i < 8; i++) {
                    board[i][j] = new Pawn(i, j, green, board);
                }
            }
        try (var listener = new ServerSocket(59090)) {
            ...
            while (true) {
                pool.execute(new Pawn(listener.accept(), "Player1"));
                pool.execute(new Pawn(listener.accept(), "Player2"));
            }
        }
    }
    ...
    public static void changeBoard(int startX, int startY, int endX, int endY) {
        board[endX][endY] = new Pawn(endX, endY, board[startX][startY].isWhite, board);
